...
class AtlasHybridPathPlanner(PathPlanner) :
    ...
    def __init__(self, robot_name, config_file):
        ...
        rospy.set_param("~atlas/manipulation_mode", "moveit")
        ...
        try :
            rospy.loginfo("AtlasHybridPathPlanner::init() -- waiting for visualization service")
            self.connected_to_plan_viz = rospy.wait_for_service('/planned_manipulation/visualize', 3.0)
            if not self.connected_to_plan_viz :
                rospy.logwarn("AtlasHybridPathPlanner::init() -- timeout out waiting for visualization service")  
        except :
            rospy.logwarn("AtlasHybridPathPlanner::init() -- timeout out waiting for visualization service")  
    ...
    def lookup_controller_name(self, group_name) :
        ...
        print "lookup_controller_name()"
        print self.group_controllers.keys()
        print self.config_package
    ...
    def execute_walk_controller_from_steps(self, footsteps, lift_heights, feet) :
        ...
        # rospy.loginfo("AtlasHybridPathPlanner::execute_walk_controller_from_steps() -- waiting for walk path result")
        ...
        #         fb_msg = rospy.wait_for_message("/path_walker/feedback", walk_controller.msg.WalkPathActionFeedback, 3.0)
        # except :
        #     rospy.logwarn("AtlasHybridPathPlanner::execute_walk_controller_from_steps() -- timeout on sending goal to walk")
    ...
    def execute_auto_walker(self, footsteps, lift_heights, feet) :
        ...
        # except :
        #     rospy.logwarn("AtlasHybridPathPlanner::execute_auto_walker() -- timeout on sending goal to walk")
    ...
    def plan_cartesian_paths(self, group_names, paths) :
        ...
        self.manipulation_mode = rospy.get_param("~atlas/manipulation_mode")
        if self.manipulation_mode == "atlas" :
            return self.plan_cartesian_paths_atlas(group_names, paths)
        else :
    ...
    def plan_cartesian_paths_atlas(self, group_names, paths) :
        ...
        rospy.loginfo("AtlasHybridPathPlanner::plan_cartesian_paths()")

        rospy.loginfo("AtlasHybridPathPlanner::plan_cartesian_paths() -- waiting for server")
        if not self.cartesian_reach_client.wait_for_server(rospy.Duration(2.0)) :
            rospy.logerr("AtlasHybridPathPlanner::plan_cartesian_paths() -- wait for server timeout")
            return False
        ...
        rospy.loginfo("AtlasHybridPathPlanner::plan_cartesian_paths() -- sending goal")
        # Sends the goal to the action server.
        self.cartesian_reach_client.send_goal(goal)

        rospy.loginfo("AtlasHybridPathPlanner::plan_cartesian_paths() -- polling feedback")
        ...
        rospy.loginfo("AtlasHybridPathPlanner::plan_cartesian_paths() -- PLANNING COMPLETE")
    ...
    def get_plan(self) :    
        ...
        try:
            ...
            rospy.logerr("AtlasHybridPathPlanner::get_plan_viz() -- service call failed")
            return None
        
        rospy.loginfo("AtlasHybridPathPlanner::get_plan_viz() -- publishing goal regions")
    ...
    def process_plan_viz(self, jt) :
        ...
        if group != "" :
            rospy.loginfo(str("AtlasHybridPathPlanner::get_plan_viz() -- trajectory for " + group))
